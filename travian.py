...
class TravianSetter(Travian):
    ...
    def set_upgrade(self, uid):
        ...
        self.driver.implicitly_wait(5)
        btn_upgrade = self.driver.find_element_by_css_selector('div.upgradeButtonsContainer div.section1 button')
        if 'green' in btn_upgrade.get_attribute('class').split():
            btn_upgrade.click()
            print('id={uid} upgrading.'.format(uid=uid))
        else:
            print('id={uid} unable to upgrade.')



if __name__ == '__main__':
    ts = TravianSetter('hank47', 'zc_7r4v14n', 'com 7')
    ts.login()
    ts.set_upgrade(26)
    ts.logout()
    ts.close_browser()
